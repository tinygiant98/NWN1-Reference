...
/////////////////////////////////////////////////////////
void CnrAddCraftingDevice(string sDeviceTag)
{
  object oModule = GetModule();

  // Check if the Crafting Device has slready been added
  ...
  //PrintString("AddCraftingDevice: CnrDeviceTag_" + IntToString(nDeviceCount) + " = " + sDeviceTag);
...
void CnrLoadRecipeScript(string sDeviceTag, object oHost)
{
  ...
  //ExecuteScript(sDeviceTag, OBJECT_SELF);
  ExecuteScript(sDeviceTag, oHost);
...
void CnrLoadAllDeviceRecipesFromScript()
{
  ...
  for (nIndex=1; nIndex<=nDeviceCount; nIndex++)
  {
    ...
    if (sDeviceTag != "")
    {
      //CnrLoadRecipeScript(sDeviceTag, oModule);

      // Defer processing to avoid TMI errors.
      CnrIncrementStackCount(oModule);
      AssignCommand(OBJECT_SELF, CnrLoadRecipeScript(sDeviceTag, oModule));
    }
  }
}
